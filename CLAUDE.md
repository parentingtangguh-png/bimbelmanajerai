...
- **Pekerjaan berikutnya yang diusulkan, menunggu pilihan pemilik** (saran Claude urut 1–3): (1) **panduan singkat guru** di HP (cara pakai + hal final + kapan "Belum dinilai"), tampilkan di pane dulu; (2) **uji Prompt kegiatan di ChatGPT** untuk kelas 3/4/5 anak (batas ±400 kata); (3) periksa menu utama yang tampak terpotong di pane sempit. Opsional: rapor/laporan orang tua, periksa cadangan data Supabase.
...
- Yang masih terbuka (hanya bila pengguna meminta): tombol "Tes diagnostik sekarang" setelah menambah siswa; menu utama di pane sempit tampak terpotong di kanan (belum diperiksa apakah juga terjadi di HP); di HP pemilik kotak konfirmasi `confirm()` saat Simpan final sempat tidak terlihat walau tersimpan benar.
