...
- **Fungsi**: `is_member`, `is_owner`, `can_teach`, `handle_new_user`, `check_student_identity`, `create_student`, `update_student_profile`, `set_student_active`, `delete_student`, `reject_owner_student_insert` (pemicu), `save_diagnostic`, `save_schedule`, `delete_schedule`.
- **Jadwal kelas (tahap 1, migrasi `20260914000000_class_schedules.sql`)**: `class_schedules(teacher_id, meeting_number 1–192, theme_number, scheduled_date, scheduled_time, completed_at)` + `class_schedule_students(schedule_id, student_id, level, indicator_number)` (level diambil dari `students.pilot_level` saat disimpan; indikator harus dari level itu). `save_schedule(p_schedule, p_payload)` buat/ubah, `delete_schedule`; keduanya hanya guru pemilik jadwal dan menolak jadwal yang `completed_at` terisi. Guru membaca jadwalnya sendiri, pemilik membaca semua. Menghapus siswa ikut mengeluarkannya dari jadwal.
...
- **Ruang kelas — tahap 1 sudah dibangun (14 Sep 2026): jadwal pertemuan.** Formulir (urutan diputuskan pemilik): **Siswa** (dropdown pilih banyak; yang belum dites tidak bisa dicentang) → **Tema** → **Indikator per siswa** (dropdown dari level siswa masing-masing; **tidak ada dropdown CP dan Level**) → **Pertemuan** (dropdown tampil tetapi **terkunci**; nomor ditentukan database) → **Tanggal**, **Jam**. Daftar dibagi **Akan datang** (Buka · Ubah · Hapus) / **Selesai** (Lihat); Ubah/Hapus hanya sebelum selesai.
- **Urutan pertemuan dijaga database (migrasi `20260914020000_meeting_order.sql`, keputusan pemilik 14 Sep 2026)**: jadwal baru = nomor terbesar milik guru + 1 (isian nomor diabaikan, maks 192); unik per guru; ubah jadwal tidak mengganti nomor; `complete_schedule` menolak bila ada nomor lebih kecil yang belum selesai ("Selesaikan Pertemuan N lebih dulu"); `delete_schedule` hanya untuk jadwal terakhir (nomor terbesar, belum selesai) supaya nomor tidak berlubang — tombol Hapus hanya muncul di jadwal itu.
- **Ruang kelas — tahap 2 sudah dibangun (14 Sep 2026, migrasi `20260914010000_class_results.sql`)**: **Buka** jadwal → setiap siswa **Lulus / Belum / Tidak hadir** (wajib semua; `class_schedule_students.result` = `lulus`/`belum`/`absen`) → **Tandai pertemuan selesai** (`complete_schedule`, isi `completed_at`, lalu dikunci). Tidak hadir = indikator tetap, tercatat berbeda. **Indikator yang sudah lulus** tampil di profil siswa (level saat ini; `passedIndicators` di `state.js`, hanya dari pertemuan selesai). Jadwal berikutnya **menyarankan indikator terkecil yang belum lulus** (`suggestedIndicator`), **guru tetap boleh mengganti** (keputusan pemilik). Jadwal selesai tidak bisa dihapus lewat aplikasi. Data produksi 14 Sep 2026: satu jadwal Pertemuan 1 (5 siswa, belum selesai) buatan pemilik saat uji; jadwal percobaan Claude sudah dihapus. **Belum**: "siap naik" + konfirmasi guru (butir 5), rapor, aktivitas kelas, catatan English & karakter.
