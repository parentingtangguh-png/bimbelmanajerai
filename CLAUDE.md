...
- **Kelas pilot (migrasi `20260914000000`…`20260914040000`)**: `class_schedules(teacher_id, meeting_number 1–192 unik per guru, theme_number, scheduled_date, scheduled_time, completed_at)` + `class_schedule_students(schedule_id, student_id, level, indicator_number, result lulus|belum|null)`. RPC: `save_schedule(p_schedule, p_payload{scheduled_date, scheduled_time})` (nomor = terbesar + 1, tema dari nomor, **hanya satu jadwal terbuka per guru**; ubah hanya tanggal/jam), `delete_schedule` (jadwal terakhir yang belum selesai), `save_meeting(p_schedule, p_students[{student_id, result}], p_finish)` (level dan indikator dihitung database lewat `current_indicator`, bukan dari aplikasi; final menolak bila ada siswa tanpa hasil atau pertemuan sebelumnya belum selesai). `current_indicator(student)` = indikator 1–6 terkecil di level siswa yang belum Lulus di pertemuan selesai, atau 6 bila semua lulus (bukan RPC). Guru membaca jadwalnya sendiri, pemilik membaca semua. Menghapus siswa ikut mengeluarkannya dari pertemuan.
...
- **Ruang kelas (dibangun 14 Sep 2026, keputusan pemilik).** Guru **tidak memilih pertemuan, tema, level, atau indikator**. **Buat jadwal** hanya **Tanggal** + **Jam** (keterangan "Pertemuan N · Tema X" otomatis); tombol Buat jadwal hilang selama ada jadwal terbuka. Saat kelas guru **Buka** jadwal → centang **siswa yang hadir** (siswa tidak terikat jadwal; terlambat tinggal dicentang; belum dites tidak bisa) → tiap siswa tampil "Level X · Indikator N. …" (teks, tanpa dropdown) + **Lulus / Belum** (tanpa Tidak hadir) → **Simpan sementara** atau **Tandai pertemuan selesai** (final, **tanpa koreksi**). Lulus → indikator berikutnya; Belum → tetap. **Antrean hanya indikator 1–6**; English (7) & Karakter (8) mewarnai pertemuan. Setelah 1–6 lulus siswa **tetap di indikator 6** dengan tanda "siap naik" sampai kenaikan level dikonfirmasi (belum dibangun). Satu siswa boleh ikut beberapa pertemuan. Daftar: **Akan datang** (Buka · Ubah · Hapus) / **Selesai** (Lihat).
