...
- **Belum diluncurkan.** Produksi berisi 6 akun tim; 0 siswa, tes, dan jadwal (data percobaan dihapus 15 Sep 2026 dengan izin pemilik).

## ▶ Mulai di sini: status terakhir (16 Sep 2026)
- **Rencana aplikasi** [`docs/rencana-aplikasi.md`](docs/rencana-aplikasi.md): Fase 1 (isi kurikulum + halaman Kurikulum), 2 (tes diagnostik baru), 3 (Ruang kelas baru), 4 (pembersihan kurikulum pilot) **sudah di produksi**. **Berikutnya Fase 5: tombol "Prompt kegiatan"** — teks prompt disesuaikan dengan kurikulum 8 level dan **diajukan sebagai draf teks kepada pemilik dulu**.
- Belum dibangun (hanya bila pemilik meminta): rapor, catatan English & karakter di luar Lulus/Belum, aktivitas kelas.
- Uji coba dengan anak sungguhan belum dilakukan. Jalur sesi selesai, antrean maju, syarat paparan English, dan naik level baru teruji di tes otomatis database (di situs live baru diuji simpan sementara, lalu dibersihkan).
...
- **Ruang kelas** (situasi acuan: 1 guru, ±10 siswa, 2 sesi/hari, 5 siswa/sesi). Guru **tidak memilih pertemuan, tema, level, atau indikator**.
  ...
  - Lembar sesi: panel **Tema hari ini** (subtema, benda nyata, kosakata Indonesia dan English, situasi Karakter) → centang **siswa hadir** (hanya anak dengan tes final; yang sudah di sesi lain pada tanggal sama disembunyikan; **satu anak satu sesi per tanggal**) → tiap anak **satu indikator akademik aktif** + cara uji/bahan/tanda lulus + **Lulus / Belum / Belum dinilai**, lalu **English** dan **Karakter** (Tidak dinilai / Lulus / Belum).
  ...
  - Sesi selesai tidak bisa dihapus lewat aplikasi — uji coba di produksi cukup sampai simpan sementara, lalu hapus.
...
- **Prompt kegiatan** (Fase 5, belum dibangun): tombol di lembar sesi (aktif setelah siswa dicentang; salin prompt ke ChatGPT/Gemini). Teks versi kurikulum pilot pernah disetujui setelah 5 kali uji (data kelas + per siswa: nama panggilan, usia, kelas formal, level, sudah lulus, kondisi sebelumnya → hari ini dilatih, cara tes & tanda lulus, English & karakter; 9 aturan termasuk waktu 0–10/10–50/50–60, urutan pendampingan bermenit, tanpa cetak, indikator tidak naik bila Belum, ±400 kata). Harus disesuaikan ke kurikulum 8 level dan disetujui ulang.
...
- Layar di `src/views/`: `dashboard.js` (Ringkasan: siswa aktif, sudah/belum dites, sebaran level), `students.js` (daftar, profil, hasil tes, level saat ini, kemajuan & naik level, status, hapus), `diagnostic.js` (modal tes: `diagnostic-start` → lembar 14 tugas `diagnostic` / tes berhenti; `state.diagnostic` = id anak yang lembarnya terbuka), `sessions.js` (Ruang kelas: hari/sesi, lembar sesi, tema hari ini, `finishState`), `curriculum.js`, `team.js`, `home.js` (menu, kepala, doa, navbar HP).
...
- Yang masih terbuka (hanya bila pengguna meminta): uji coba dengan anak sungguhan; tombol "Tes diagnostik sekarang" setelah menambah siswa; menu utama di pane sempit tampak terpotong di kanan (belum diperiksa apakah juga terjadi di HP).
