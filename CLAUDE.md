...
- **Belum diluncurkan.** Data yang ada adalah akun tim dan siswa percobaan milik satu guru uji.

## ▶ Mulai di sini: status terakhir (14 Sep 2026, akhir sesi)
**Sudah di produksi** (commit terakhir `16faeb3`, migrasi terakhir `20260914050000_meeting_per_day.sql`): arsitektur pilot + **Ruang kelas** (jadwal per hari dengan sesi, siswa hadir, Lulus/Belum, indikator otomatis 1–6). Semua tes, build, Playwright hijau saat deploy.

**Data produksi percobaan (14 Sep 2026):** 10 siswa milik satu guru uji, seluruhnya sudah menjalani diagnostik. Distribusi level: L1=2, L2=2, L3=3, L4=3. Pemilik mungkin punya jadwal/sesi percobaan di Ruang kelas. **Pemilik sudah memutuskan: saat kurikulum baru dipasang, seluruh data siswa percobaan beserta tes dan kelas terkait dihapus** setelah target query ditunjukkan kepada pemilik.
...
- **Dikunci 14 Sep 2026:** arsitektur, Matriks Cakupan Bahan beserta amendemen (`docs/curriculum/matriks-cakupan-bahan.md`), dan 96 milestone (`docs/curriculum/milestone-12x8.md`). Perubahan hanya lewat keputusan eksplisit pemilik. **96 indikator operasional selesai dan dikunci** (Alur A–F, `docs/curriculum/indikator/alur-a.md` … `alur-f.md`; A–E 14 Sep, F 15 Sep 2026). **English & Karakter:** matriks + milestone dan 16 indikator dikunci 15 Sep 2026 (`docs/curriculum/english-karakter.md`, `docs/curriculum/indikator/english-karakter.md`). Seluruh 112 indikator selesai. **Aturan tes diagnostik baru dikunci 15 Sep 2026** (`docs/curriculum/diagnostik.md`; menggantikan aturan diagnostik di aplikasi, yang belum diubah). **CP, level, dan tema dikunci 15 Sep 2026** (`docs/curriculum/cp-level-tema.md`; tema = konteks bersama kelas multi-level; English kini per tema, amendemen di `english-karakter.md`, `indikator/english-karakter.md`, `diagnostik.md`, arsitektur §10). **Isi kurikulum selesai. Rencana aplikasi disetujui (`docs/rencana-aplikasi.md`, 5 fase). Fase 1 di produksi 15 Sep 2026:** tabel `k8_*` (migrasi `20260915000000_curriculum_8_levels.sql`, dibangkitkan `node scripts/build-curriculum.mjs` dari docs; jangan sunting tangan, `tests/curriculum-docs.test.mjs` menjaga kesamaannya) dan halaman Kurikulum 8 level (`src/markdown.js` untuk isi dokumen). **Fase 2 di produksi 15 Sep 2026** (migrasi `20260915010000_diagnostic_8_levels.sql`): data siswa percobaan dihapus (0 siswa/tes/sesi; 6 akun tetap); level 1–8; tes diagnostik baru — `start_diagnostic`, `rate_diagnostic` (tiap ketukan, draf di server, nilai boleh diubah sebelum final kecuali tes berhenti), `finalize_diagnostic` (start_level/start_indicator/curriculum_complete); `diagnostic_results(test_id, number 1–14 kecuali 13, status lulus|belum, package)`; `delete_student` ditolak bila anak pernah ikut kelas; **Ruang kelas dikunci** (UI + `save_schedule`/`delete_schedule`/`save_meeting` menolak). Bagian "Arsitektur sekarang"/"Tes Diagnostik" di bawah yang menyebut level 1–4, T/B/N, localStorage sudah usang; dirapikan di Fase 4. **Fase 3 di produksi 16 Sep 2026** (migrasi `20260916000000_classroom_8_levels.sql`): Ruang kelas dibuka lagi untuk 8 level — tema/subtema dari `k8_themes`; `class_schedule_students(level 1–8, indicator_number 1–12 atau null bila siap naik, result lulus|belum|belum_dinilai, english_result, character_result)`; `passed_numbers` (sesi selesai + diagnostik final level itu), `current_indicator` (null = 12 Lulus), `theme_attendance`; English L2/L3/L5/L6/L8 butuh hadir ≥3 hari dalam satu tema **termasuk hari ini**, L1/L4/L7 bebas; EN/KR lulus sekali per level dan tidak menahan naik level; `confirm_level_up` + tabel `student_level_changes`. Profil guru: "Kemajuan Level X". **Pemilik minta setiap hal yang perlu disetujui ditampilkan di pane** (halaman pratinjau data contoh). **Berikutnya Fase 4 (pembersihan).**
...
**Juga sudah dirancang tetapi belum dibangun — tombol "Prompt kegiatan"** di lembar sesi (aktif setelah siswa dicentang; salin prompt ke ChatGPT/Gemini). Teks prompt disetujui setelah 5 kali uji (versi terakhir: data kelas + per siswa: nama panggilan, usia, kelas formal, level, sudah lulus, kondisi sebelumnya → hari ini dilatih, cara tes & tanda lulus dari kurikulum, English & karakter level; 9 aturan termasuk waktu 0–10/10–50/50–60, urutan pendampingan bermenit, tanpa cetak, indikator tidak naik bila Belum, ±400 kata). **Tunda sampai kurikulum baru dipasang** karena isi prompt bergantung pada indikator.

**Belum dibangun lainnya:** konfirmasi naik level ("siap naik"), catatan English & karakter, rapor. Keputusan L1-1/L1-2 (indikator dekat karakter) diserap ke kurikulum baru.
...
- **Rancangan awal alur kelas pilot (13 Sep 2026) — sebagian sudah digantikan oleh Ruang kelas yang dibangun 14 Sep (pertemuan per hari, bukan per guru; tema otomatis; indikator otomatis). Bila bertentangan, ikuti butir "Ruang kelas (dibangun 14 Sep 2026)" di atas** — situasi acuan: 1 guru, ±5 anak beda level, satu tema, sesi 60 menit. Frekuensi pertemuan urusan guru; aplikasi tidak boleh berasumsi jadwal.
...
- Layar di `src/views/`: `dashboard.js` (Ringkasan: siswa aktif, sudah/belum dites, sebaran level), `students.js` (daftar, profil, hasil tes, level saat ini, status, hapus), `diagnostic.js` (modal tes: `diagnostic-start` → `diagnostic-level` → hasil `diagnostic`, dibedakan `run.reviewed`), `sessions.js` (Ruang kelas: hari/sesi, lembar sesi, `finishState`), `curriculum.js`, `team.js`, `home.js` (menu, kepala, doa, navbar HP).
