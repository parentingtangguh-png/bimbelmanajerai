...
- **Kelas pilot (migrasi `20260914000000`…`20260914030000`)**: `class_schedules(teacher_id, meeting_number 1–192 unik per guru, theme_number, scheduled_date, scheduled_time, completed_at)` + `class_schedule_students(schedule_id, student_id, level, indicator_number, result lulus|belum|null)` (level diambil dari `students.pilot_level` saat disimpan; indikator harus dari level itu). RPC: `save_schedule(p_schedule, p_payload{theme_number, scheduled_date, scheduled_time})` buat/ubah jadwal, `delete_schedule`, `save_meeting(p_schedule, p_students[{student_id, indicator_number, result}], p_finish)`. Hanya guru pemilik jadwal; jadwal yang `completed_at` terisi ditolak. Guru membaca jadwalnya sendiri, pemilik membaca semua. Menghapus siswa ikut mengeluarkannya dari pertemuan.
...
- **Ruang kelas (dibangun 14 Sep 2026).** **Buat jadwal** hanya: **Tema** → **Pertemuan** (dropdown tampil tetapi **terkunci**) → **Tanggal**, **Jam**. **Siswa tidak terikat jadwal** (keputusan pemilik): saat kelas guru **Buka** jadwal → **Siswa yang hadir** (dropdown pilih banyak; siswa terlambat tinggal dicentang; belum dites tidak bisa; tanpa dropdown CP/Level) → untuk tiap siswa **dropdown indikator** dari levelnya (saran = terkecil yang belum lulus via `suggestedIndicator`, guru boleh mengganti) + **Lulus / Belum** (tanpa Tidak hadir: yang tidak dicentang memang tidak hadir) → **Simpan sementara** (hasil boleh kosong, bisa diubah) atau **Tandai pertemuan selesai** (final, dikunci; minimal satu siswa, semua dinilai). Satu siswa boleh ikut beberapa pertemuan. Daftar: **Akan datang** (Buka · Ubah · Hapus) / **Selesai** (Lihat).
- **Urutan pertemuan dijaga database (migrasi `20260914020000_meeting_order.sql`, keputusan pemilik 14 Sep 2026)**: jadwal baru = nomor terbesar milik guru + 1 (isian nomor diabaikan, maks 192); unik per guru; ubah jadwal tidak mengganti nomor; `save_meeting` (final) menolak bila ada nomor lebih kecil yang belum selesai ("Selesaikan Pertemuan N lebih dulu"); `delete_schedule` hanya untuk jadwal terakhir (nomor terbesar, belum selesai) supaya nomor tidak berlubang — tombol Hapus hanya muncul di jadwal itu.
