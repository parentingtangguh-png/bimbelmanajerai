...
- **Mulai 17 Sep 2026 guru memakai aplikasi dengan siswa nyata.** Pekerjaan berikutnya berangkat dari catatan guru, bukan dari fitur baru.
- **Pengingat untuk guru (disampaikan pemilik):** (1) tambah siswa lalu tes diagnostik sampai Simpan final — tanpa itu anak tidak bisa dicentang di kelas; (2) Simpan final, Tandai sesi selesai, Naik ke Level tidak bisa dibatalkan — ragu: Jeda / Simpan sementara; (3) salah input tes final: hapus siswa lalu tambah ulang, hanya sebelum anak ikut kelas; (4) di kelas: centang hadir → Prompt kegiatan (kehadiran tersimpan otomatis) → obrolan baru ChatGPT/Gemini → nilai memakai Cara uji di lembar sesi → Tandai sesi selesai; (5) tombol 📖 Panduan di setiap tab.
- **Minggu pertama:** atas permintaan pemilik, periksa kejanggalan lewat query baca-saja dan laporkan **jumlah/status saja**, tanpa nama anak (tes draf yang tertinggal, sesi belum ditandai selesai, siswa aktif tanpa tes final). Contoh: `select (select count(*) from diagnostic_tests where finalized_at is null) tes_draf, (select count(*) from class_schedules where completed_at is null) sesi_terbuka, (select count(*) from students s where status='Aktif' and not exists (select 1 from diagnostic_tests t where t.student_id=s.id and t.finalized_at is not null)) aktif_tanpa_tes_final`. Kumpulkan catatan guru (bagian yang membingungkan, prompt yang tidak cocok untuk kelas).
- **Cadangan data (diperiksa 16 Sep 2026):** `supabase backups list` → `backups: []`, `pitr_enabled: false`. Artinya tidak ada cadangan harian yang bisa dipulihkan; Pemilik memilih **cadangan manual** (16 Sep 2026). Docker/pg_dump tidak ada, jadi `node scripts/backup-data.mjs "D:ibuan_penggunaCLAUDEimbel_cadangan"` membaca 9 tabel aplikasi lewat `db query` (hanya baca) ke folder bertanggal WIB berisi JSON per tabel, `ringkasan.json` (jumlah baris), dan `restore.sql`. Folder cadangan **di luar repo** (berisi data anak; skrip menolak folder di dalam repo). Kurikulum dan auth.users tidak dicadangkan (restore mengandaikan akun dengan id sama). `tests/backup.test.mjs` membuktikan restore.sql memulihkan semua tabel apa adanya di PGlite. **Jalankan restore hanya dengan izin pemilik.** Cadangan pertama 16 Sep 2026 (0 siswa, 6 akun). Juga diuji: cadangan produksi asli dipulihkan ke PGlite → 9 tabel sama (bandingkan tanpa urutan kolom; JSON dari CLI mengurutkan kunci). Belum diuji: pemulihan ke Supabase sungguhan dan cadangan berisi siswa nyata — setelah hari mengajar pertama, buat cadangan lalu uji pulih ke PGlite (laporkan jumlah saja). Frekuensi yang disarankan: setiap akhir hari mengajar, atas permintaan pemilik.
...
## Menguji
- `npm test` → `scripts/check-imports.mjs`, lalu tes Node: `curriculum-docs` (migrasi kurikulum = hasil pembangkit dari docs), `domain`, `backup` (restore.sql memulihkan data contoh), `database` (PGlite menjalankan **semua** migrasi, jadi yang diuji keadaan akhirnya), dan `render` (layar disusun dengan `tests/fixtures/sample-state.mjs`, tanpa browser).
