...
import { state, subjectLabels, activeCompetenciesFor, ready, timeLabel, scheduleMembers, durationText, areaProgress, phaseOf, phaseShort, phasePct, levelOptions } from '../state.js';
...
export function competencyTargetsForm(s){const rows=activeCompetenciesFor(s.id);return rows.length?`<form data-form="competency-targets" data-id="${s.id}"><h3>Target per kompetensi</h3><p class="muted">Empat elemen Bahasa Indonesia, Matematika, dan IPAS wajib. English Exposure tetap pengayaan.</p><div class="form-grid">${rows.map(c=>field(subjectLabels[c.subject],`target_${c.subject}`,'number',c.target,`required min="${c.current_level}" max="16"`)).join('')}</div><button class="secondary">Simpan target kompetensi</button></form>`:'';}
