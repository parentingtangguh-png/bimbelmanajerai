  ...
  const evaluation = targets
    .map(a => {
      ...
      const checks = list.length
        ? `<h4>Indikator</h4><div class="indicator-checks" data-key="${key}" data-record="${r.id}" data-subject="${a.subject}">${list.map((t, i) => `<label class="check"><input type="checkbox" class="indicator-check" data-index="${i + 1}" data-text="${h(t)}" ${saved.has(i + 1) ? 'checked' : ''} ${locked || absent ? 'disabled' : ''}>${i + 1 === key ? '<span class="star">★</span> ' : ''}${i + 1}. ${h(t)}${before.has(i + 1) ? ' <em class="seen">sudah pernah</em>' : ''}</label>`).join('')}<p class="indicator-hint">${hintText(done, list.length, key, keyDone)}</p>${history}</div>`
...
export function newSession() {
  ...
  modal(
    'Mulai sesi kelas',
    `<form data-form="session" data-id="${crypto.randomUUID()}">${field('Tanggal', 'date', 'date', localDate(), 'required')}${state.schedules.length ? select('Sesi jadwal', 'schedule', [['', 'Tanpa sesi jadwal'], ...state.schedules.map(sc => [sc.id, `${sc.name} · ${timeLabel(sc)}`])], '') : ''}${select(
      'Durasi kelas',
      'duration',
      [
