...
export const initialState = () => ({ user:null, role:'teacher', name:'', view:'dashboard', students:[], classes:[], records:[], competencies:[], assessments:[], observations:[], indicatorChecks:[], alerts:[], members:[], assignments:[], profiles:[], themes:[], curriculum:[], active:null, demo:false, filter:'', schedules:[], scheduleStudents:[], curriculumTab:'strand', curriculumSubject:'reading', curriculumPhase:'all' });
export const state = initialState();
export const icons={dashboard:'◫',students:'◉',sessions:'▤',team:'♧',curriculum:'▥'};
export const labels={dashboard:'Ringkasan',students:'Data siswa',sessions:'Ruang kelas',team:'Tim pengajar',curriculum:'Kurikulum'};
...
export const subjectLabels={listening:'Menyimak',speaking:'Berbicara',reading:'Membaca',writing:'Menulis',math:'Matematika',ipas:'IPAS',english:'English Exposure'};
...
export const reminderOfTheDay=()=>reminders[Math.floor(Date.parse(localDate())/86400000)%reminders.length];
export const characterLabels={kemandirian:'Kemandirian',tanggung_jawab:'Tanggung jawab',kerja_sama:'Kerja sama',kepedulian:'Kepedulian',komunikasi_santun:'Komunikasi santun'};
...
export const hintText=(done,total,key,keyDone)=>done===0
...
export const observationFor=id=>state.observations.find(o=>o.session_student_id===id)||{english_rating:null,english_note:'',character_dimensions:[],character_note:''};
export const activeCompetenciesFor=id=>competenciesFor(id).filter(c=>c.active!==false&&subjects.includes(c.subject));
export const ready=s=>{const rows=activeCompetenciesFor(s.id).filter(c=>c.required);return rows.length?rows.every(c=>c.current_level>=c.target):s.reading_level>=s.reading_target&&s.math_level>=s.math_target;};
export const timeLabel=x=>x?.start_time?`${formatTime(x.start_time)}–${formatTime(x.end_time)}`:'';
export const scheduleMembers=id=>new Set(state.scheduleStudents.filter(x=>x.schedule_id===id).map(x=>x.student_id));
export function durationText(start,end){if(!start||!end)return 'Isi jam mulai dan jam selesai.';const mins=minutesBetween(start,end);if(mins<30||mins>180)return 'Durasi harus 30–180 menit.';return `${mins} menit · kelas memakai pola ${durationPattern(mins)} menit.`;}
...
export function indicatorsOf(c,k){const v=c&&c[`${k}_indicators`];if(Array.isArray(v))return v.filter(x=>typeof x==='string'&&x.trim());if(typeof v==='string'&&v.trim()){try{const parsed=JSON.parse(v);return Array.isArray(parsed)?parsed:[];}catch{return [];}}return [];}
export const phaseOf=level=>level<=4?'Fondasi (belum SD)':level<=8?'Fase A · SD 1–2':level<=12?'Fase B · SD 3–4':'Fase C · SD 5–6';
export const phaseShort=level=>level<=4?'Fondasi':level<=8?'Fase A':level<=12?'Fase B':'Fase C';
...
export const levelOptions=[...Array(16)].map((_,i)=>[String(i+1),`Level ${i+1} · ${phaseOf(i+1)}`]);
