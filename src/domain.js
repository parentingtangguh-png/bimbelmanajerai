...
export function minutesBetween(start, end) {
  const toMinutes = t => { const [hour, minute] = String(t).split(':').map(Number); return hour * 60 + minute; };
  return toMinutes(end) - toMinutes(start);
}
// Class patterns are 60/75/90 minutes; pick the one closest to a schedule slot.
export function durationPattern(minutes) { return minutes < 70 ? 60 : minutes < 85 ? 75 : 90; }
export function formatTime(value) { return value ? String(value).slice(0, 5).replace(':', '.') : ''; }
