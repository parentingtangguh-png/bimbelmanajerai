-- Group by competency bands, not by equal headcount. Students at the same
-- competency position must stay together even when that makes group sizes uneven.
...
begin
  ...
  with scores as (
    ...
    select min(score) as minimum,max(score) as maximum from scores
  ), banded as (
    select scores.id,case
      when bounds.minimum=bounds.maximum then 1
      else least(3,1+floor((scores.score-bounds.minimum)*3/(bounds.maximum-bounds.minimum))::integer)
    end as band
    from scores cross join bounds
  ), grouped as (
    select banded.id,dense_rank() over(order by banded.band)::smallint as group_no
    from banded
  )
