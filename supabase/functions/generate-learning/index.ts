...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const data={nama:s.name,sapaan:s.parent_name,minat:s.interest,gaya_belajar:s.learning_notes,tema:c.theme,level_membaca:r.reading_snapshot,level_matematika:r.math_snapshot,
      bank:{membaca:read?.reading,menulis:read?.writing,matematika:math?.math,inggris:read?.english,karakter:read?.character,ipas:read?.ipas,pancasila:read?.pancasila},remedial,variasi_sebelumnya:remedial?prev?.material?.slice(0,1800):undefined,
      ...(body.kind==='report'?{nilai:r.grade,catatan:r.anecdote,panduan:r.material,progres_membaca:bar(readingAfter,s.reading_baseline,s.reading_target),progres_matematika:bar(mathAfter,s.math_baseline,s.math_target)}:{})};
